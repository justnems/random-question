...
function setResult(categoryKey, questionText) {
  var categoryDisplay = document.getElementById("categoryDisplay");
  var questionDisplay = document.getElementById("questionDisplay");

  if (categoryDisplay) {
    categoryDisplay.innerHTML = "Category: " + formatCategoryLabel(categoryKey);
  }
...
function getValidCategories(categoryKeys) {
  var allCategories = Object.keys(questions);
  if (!categoryKeys || categoryKeys.length === 0) {
    return allCategories;
  }

  return categoryKeys.filter(function (key) {
    return allCategories.indexOf(key) !== -1 && questions[key].length > 0;
  });
}

function newQuestion(categoryKeys) {
  var normalized = Array.isArray(categoryKeys)
    ? categoryKeys
    : categoryKeys
      ? [categoryKeys]
      : [];
  var validCategories = getValidCategories(normalized);

  if (validCategories.length === 0) {
    return;
  }

  var selectedCategory = pickRandomItem(validCategories);
  var selectedQuestion = pickRandomItem(questions[selectedCategory]);
  setResult(selectedCategory, selectedQuestion);
}
...
function populateCategorySelect() {
  ...
  var optionKeys = Object.keys(questions);
...
function newCategoryQuestion() {
  var select = document.getElementById("categorySelect");
  var selectedCategory = select ? select.value : "";
  if (selectedCategory) {
    newQuestion(selectedCategory);
    return;
  }
